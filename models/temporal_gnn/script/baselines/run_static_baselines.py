...
class Runner(object):
    ...
    def train(self):
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            loss = self.model.compute_loss(z, train_pos_edge_index, train_neg_edge_index)
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    init_logger(mkdirs(args.output_folder + '/log/') + args.dataset + '_seed_' + str(args.seed) + '.txt')
    run = Runner()
    run.train()
